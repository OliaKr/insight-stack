...
def fetch_report(endpoint_name):
    """
    Fetches report text from the MCP server by endpoint name.
    """
...
coordinator_agent = AssistantAgent(
    ...
    system_message=(
        "You are the CoordinatorAgent responsible for generating **exactly 5 distinct strategic insights** "
        ...
        "Only generate one block of five insights. Do not respond again unless explicitly prompted. "
...
# Fetch reports via MCP
sales_text = fetch_report("sales-report")
support_text = fetch_report("support-report")
ops_text = fetch_report("ops-report")


def query_kpi(kpi: str):
    try:
        response = requests.post(
            "http://localhost:8000/search-kpi",
            json={"kpi": kpi}
        )
        response.raise_for_status()
        result = response.json().get("matches", [])
        return "\n".join(result) if result else f"No data found for '{kpi}'"
    except Exception as e:
        return f"Error querying KPI tool: {e}"


conversion_insights = query_kpi("conversion")
print("\n🧪 Conversion KPI from Tool:\n", conversion_insights)
...
def coordinator_responded_once(messages):
    return sum(
        ...
        if isinstance(msg, BaseChatMessage) and msg.source == "CoordinatorAgent"
    ) >= 1
...
async def main():
    ...
    count = sum(
        1 for msg in result.messages if msg.source == "CoordinatorAgent")
    ...
    insights = [
        msg.content
        for msg in result.messages if msg.source == "CoordinatorAgent"
    ]
